      ...
      for subIndex, subordinate in pairs(currentSubordinates) do
         if (subordinate:isDisaggregatedUnit()) then
            mySubordinateTasks[subordinate:getUUID()] = vrf:sendTask(subordinate, 
		       "disaggregate-find-cover-point", {findCoverFrom = findCoverFrom.findCoverFrom, radius = taskParameters.radius})
         else
            mySubordinateTasks[subordinate:getUUID()] = vrf:sendTask(subordinate, 
		       "find-cover-point", {findCoverFrom = taskParameters.findCoverFrom, radius = taskParameters.radius})
	end
      end
   end
end
