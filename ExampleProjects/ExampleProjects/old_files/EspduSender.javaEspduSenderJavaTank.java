...
* @author DMcG
*/
public class EspduSenderJavaTank
{
...
double lat = 29.517024;//36.595517;
double lon = 46.555961;//-121.877000;
double alt =250;
...
eid.setEntityID(302);//setEntity(302);
...
entityType.setSpecific((short)3);//setSpec((short)3); // M1A2 Abrams
Set<InetAddress> bcastAddresses = getBroadcastAddresses();
EntityMarking marking = espdu.getMarking();
byte[] VRmarking = new byte[11];
VRmarking ="Java Tank".getBytes();
marking.setCharacters(VRmarking);
...
int ts = disTime.getDisRelativeTimestamp();//AbsoluteTimestamp();
...
//System.out.print ("Marking: "+ espdu.getMarking());
...
System.out.println("Espdu #" + idx + " EID=[" + eid.getSiteID()+ "," + eid.getApplicationID()+ "," + eid.getEntityID()+ "]");
