...
public class Simulation extends Thread {
	    	...
	    	try {
	    		...
	    		while(true) {
	    			...
	    			timestamp = dt.getDisRelativeTimestamp();
	    			 int delta = Math.abs(timestamp-preTs);
	    		    	 ...
	    		    	 /*for (Enumeration<EntityStatePdu> e = dataObj.getRemoteEspdus().elements(); e.hasMoreElements();) {
	    		 		    ...
	    		 		    printLocation(temp);
	    		 		    System.out.println(temp.getDeadReckoningParameters().getDeadReckoningAlgorithm());
	    		 		   double[] r = dataObj.getM_dr().get("1300246").getUpdatedPositionOrientation();
	    		 		   System.out.println(dataObj.getM_dr().get("1300246").toString());
	    		 		    //System.out.println("dr: ");
	    		 		    //getDR(timestamp, temp);
	    		    ...
	    		    if((!dataObj.getRemoteEspdus().isEmpty())  ){
	    		    	 temp =  dataObj.getRemoteEspdus().get("1300246"); // get ESpdu with EID[1,3002,46] EID[siteID, applicationID,EntityID]
	    		    	 System.out.println("tank's updatetime"+temp.getTimestamp());
	    		 		 System.out.println("time now : "+ timestamp);
	    		 		 printLocation(temp);
	    		 		double[] r = dataObj.getM_dr().get("1300246").getUpdatedPositionOrientation();//get dead reckoning data of EID[1,3002,46] ESpdu
	    		 		// location x value: r[0],location y value: r[1], location z value: r[2]
	    		 		System.out.println(dataObj.getM_dr().get("1300246").toString());// print out all dead reckoning data 
	...
	public static void printLocation(EntityStatePdu e) {
		System.out
				.print(" EID=[" + e.getEntityID().getSiteID() + "," +e.getEntityID().getApplicationID() + "," + e.getEntityID().getEntityID() + "]");
		System.out.print(" DIS coordinates location=[" + e.getEntityLocation().getX() + "," + e.getEntityLocation().getY() + ","
				+ e.getEntityLocation().getZ() + "]");
